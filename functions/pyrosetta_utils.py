...
# Repository root, so the bundled DAlphaBall binary can be found without config.
_REPOSITORY_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_INITIALISED = False


def init_pyrosetta(dalphaball_path=None):
    """Initialise PyRosetta once per process.

    PackStat and BuriedUnsatHbonds need the bundled DAlphaBall binary; it is
    resolved from the repository the same way generic_utils resolves dssp.
    """
    global _INITIALISED
    if _INITIALISED:
        return
    if not dalphaball_path:
        dalphaball_path = os.path.join(_REPOSITORY_ROOT, "functions", "DAlphaBall.gcc")
    if not os.access(dalphaball_path, os.X_OK):
        raise RuntimeError(
            f"DAlphaBall is not executable: {dalphaball_path}. "
            f"Run chmod +x {dalphaball_path}."
        )
    pr.init(
...
def _interface_spec(binder_chain, target_chains):
    """Build a Rosetta interface string, e.g. ('D', ['A','B','C']) -> 'ABC_D'."""
    target = [str(chain).strip() for chain in target_chains if str(chain).strip()]
    target = [chain for chain in target if chain != binder_chain]
    if not target:
        raise ValueError("score_interface needs at least one target chain")
    return f"{''.join(target)}_{binder_chain}"


# Rosetta interface scores
def score_interface(pdb_file, binder_chain="B", target_chains=("A",)):
    # load pose
    init_pyrosetta()
    ...
    iam.set_interface(_interface_spec(binder_chain, target_chains))
    ...
    interface_residues_set = hotspot_residues(
        pdb_file, binder_chain, target_chains=list(target_chains)
    )
    ...
    # use_ddG_style asserts on `symmetric || pose.num_chains() < 4`
    # (BuriedUnsatHbondFilter.cc:530), so multi-chain targets -- e.g. a binder
    # against a three-chain pMHC -- cannot be scored with this filter. Report
    # the metric as None there and keep the rest of the interface scores.
    ...
    if interface_nres != 0 and interface_delta_unsat_hbonds is not None:
