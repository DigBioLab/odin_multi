...
def hotspot_residues(trajectory_pdb, binder_chain="B", atom_distance_cutoff=4.0,
    ...
    # Get atoms and coords for the target chain(s). The design loop merges the
    # whole target into chain A, so that stays the default; AF3 predictions keep
    # the original target chains and pass them explicitly.
    ...
    if not target_atoms:
        raise ValueError(f"No target atoms found for chains {target_chains}")
